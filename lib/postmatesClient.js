...
module.exports = class Postmates {
    ...
    CreateQuote(quoteDetails){
        ...
        for(let key in quoDetails){
            params.append(key, quoDetails[key]);
        }
        return fetch(url, {
            method: 'POST',
            credentials: 'include',
    ...
    CreateDelivery(deliveryDetails){
        ...
        let params = new URLSearchParams();
        let manifest_items = JSON.stringify(delDetails.manifest_items);
        for(let key in delDetails){
            params.append(key, delDetails[key]);
        }
